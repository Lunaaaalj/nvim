...
**Output pane (`lua/defaults/molten_pane.lua`):** Molten's output does **not** use its floating window (`molten_auto_open_output = false`). Output goes to a docked scratch buffer in the right-hand column — stacked *above* the REPL terminal when one is open (`<leader>tR`/`<leader>tp`), otherwise its own vertical split. `<leader>os` opens/focuses it, `<leader>ot` toggles, `<leader>oh` closes; it auto-refreshes after every evaluation (`molten_run`/`<leader>rr`/visual `<leader>r` call `pane.follow()`). Two mechanics matter: (1) Molten only materialises a cell's output buffer while rendering its float, so `grab()` runs `MoltenShowOutput` → copies the buffer → `MoltenHideOutput` **inside one callback**; no redraw happens in between, so the float is never visible. (2) Molten binds each figure to the *float's* window (`outputchunks.py:161` `add_image(..., bufnr, winnr)`), which is why plots don't follow the buffer into another window — the pane reads those image objects' `.path` via `require("image").get_images()` and renders its own copies bound to the pane window. `molten_image_location = "float"` keeps plots out of the code buffer so they appear only in the pane. `follow()` polls on Molten's own status header ("On Hold"/"Running" vs "Done"/"Failed", `outputbuffer.py:50`) and stops the moment every cell settles, the pane closes, or a 2-minute cap hits — it is the only repeating work, and only while a cell is actually pending.

**The pane shows every cell, not just the one under the cursor**, so it reads as a console log of the session. `MoltenGoto n` indexes Molten's own sorted cell list modulo its length (`__init__.py:441`), so walking n = 1, 2, … until the cursor returns to the first position enumerates exactly the cells that exist — including ones made by a visual selection or the evaluate operator, which a scan for ` ``` ` fences would miss. Set `require("defaults.molten_pane").all_cells = false` for the old cursor-cell-only behaviour.

Reading one cell costs ~13ms (`MoltenShowOutput` builds and tears down the float; `MoltenGoto` adds ~2ms), so re-reading everything on every poll does not scale — 40 cells is ~500ms of *synchronous* stall per refresh, the same failure mode as the freeze above. Three things keep the steady state flat, and they are why `collect_here` looks the way it does:

- **Cell output is cached and only re-read when it can have changed** — when the cell is still pending, or when the caller says it just evaluated it. `follow(dirty)` takes the list of `{first, last}` line ranges that were run (the keymaps pass what they evaluated), and `is_dirty` maps a range onto cells by treating cell *i* as owning the lines from its own start to the next cell's start. A 40-cell notebook polls at ~10-45ms instead of ~500ms.
- **Positions are cached separately from output**, keyed on the buffer's `changedtick`: editing moves cells (so the `MoltenGoto` walk must be redone) without invalidating anything they printed.
- **An empty reading is never cached.** Molten doesn't build a cell's output buffer until it has produced something, so "empty" means *queued and not started* — under `<leader>ra` the whole tail of the notebook reads that way. Caching it would permanently hide those cells; instead it counts as pending and the poll picks it up when it lands.

The poll interval is derived rather than fixed: each wait is the last refresh's cost divided by a 10% budget, clamped to 500ms–3s, so a long notebook backs off on its own instead of stalling the UI.

Two rendering details follow from that: `set_body` rewrites only the lines that differ (a full `nvim_buf_set_lines` would drop the extmarks holding the images' virtual padding, so plots would vanish and be redrawn — visible blinking every poll), and `render_images` skips the redraw entirely unless an image moved or the rewrite touched a line at/below it.

Four further details, each working around something non-obvious:

- **Plot background.** ipykernel's inline backend sets `figure.facecolor` to `(1,1,1,0)`, so matplotlib PNGs are transparent, and the Kitty protocol composites them over whatever is already on screen — dark axes on your wallpaper. `flatten()` runs `magick <src> -background <bg> -alpha remove -alpha off <dst>` into `stdpath("cache")/molten_pane/` (cached per source file, async, ~20ms) and renders *that*. ImageMagick is already a hard dependency of image.nvim's default `magick_cli` processor, so nothing new is installed; if `magick` is missing the original renders unflattened. Change the colour via `require("defaults.molten_pane").plot_background`.
- **Plot size.** image.nvim's `max_width`/`max_height` (set in `lua/plugins/molten.lua`) are **global**, applied to every image in `renderer.lua:203`; there is no per-image equivalent — passing `max_width`/`max_height` to `from_file` does nothing. That global `max_height = 12` is what made pane plots tiny. The pane sets `img.ignore_global_max_size = true` (assigned after construction — `from_file` drops the option) and computes explicit `width`/`height` in cells from `image/utils`' `term.get_size()`, scaling to fill the pane in *either* direction, up as well as down.
- **Pane background.** `MoltenPaneNormal` links to `Normal`, i.e. the pane is transparent like every other window and inherits the terminal's colour/opacity. It does not need a surface of its own because the plot background is baked into the PNG. Override the group to give it a distinct one; `NormalNC` is mapped to it too so the pane doesn't dim when unfocused.
- **Terminal docking.** The REPL terminals are toggleterm *vertical* splits, so opening one while the pane is up would produce three side-by-side columns. A `TermOpen`/`BufWinEnter` autocmd calls `vim.fn.win_splitmove()` to relocate the terminal below the pane — moving the window rather than closing it, so toggleterm's handle stays valid. Both events are needed: toggleterm displays a plain scratch buffer and only then calls `termopen()` (so `BufWinEnter` sees `buftype == ""` on first open), while re-showing an existing terminal buffer never fires `TermOpen`. Full-width (`<leader>th`) and floating (`<leader>tf`) terminals are left alone.

Because `MoltenShowOutput` reads the *current* window's buffer and cursor, `grab()` runs inside `nvim_win_call(state.src, …)`, where `state.src` is the last non-scratch window seen — otherwise refreshing from the pane or while sitting in the REPL below it silently finds no cell.
...
| `<leader>os` / `<leader>ot` | Molten output pane: open / toggle (docked split) |
...
- ImageMagick (`brew install imagemagick`) — image.nvim's default `magick_cli` processor needs it, and the Molten output pane uses it to flatten transparent plot PNGs onto a solid background. Without it plots still render, just transparent.
